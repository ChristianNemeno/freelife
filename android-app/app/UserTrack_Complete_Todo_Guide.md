...
| ☁️ Phase 6 | [06-gcp-deploy.md](./06-gcp-deploy.md) | Docker → Google Cloud Run + Cloud SQL | Week 11–12 |
...
# ☁️ Phase 6 — Deploy to Google Cloud

> **Goal:** Push your Docker image to Google Artifact Registry, run it on Cloud Run, connect it to a Cloud SQL PostgreSQL database, and point your Android app at the live production URL.
...
| GCP Service | Purpose |
|-------------|---------|
| Artifact Registry | Store Docker images (like a private Docker Hub) |
| Cloud Run | Run Docker containers — serverless, scales to zero |
| Cloud SQL (PostgreSQL) | Managed PostgreSQL database with automatic backups |
| Secret Manager | Store JWT keys and DB passwords securely |
| Cloud DNS | Custom domain (e.g. api.usertrack.com) |
| GitHub Actions | CI/CD — auto-deploy on every push to main |

> 💡 **Cloud Run is perfect for beginners** — no servers to manage, scales automatically, and has a very generous free tier. You only pay for actual usage.
...
- [ ] Create a new project:
  ...
  - Name: `usertrack`
  - Note your **Project ID** (e.g., `usertrack-461829`) — you'll use this in every command
- [ ] Enable billing on the project (requires a credit card — free tier usage won't be charged)
- [ ] Install the **gcloud CLI**:
  - Download from [cloud.google.com/sdk/docs/install](https://cloud.google.com/sdk/docs/install)
  - Run the installer, then open a terminal:
    ```bash
    gcloud init
    # Follow prompts: sign in with Google, select your usertrack project
    ```
...
- [ ] Run this once to enable everything you need:
  ```bash
...
## 6.3 Create the Cloud SQL PostgreSQL Database

- [ ] Go to **GCP Console → SQL → Create Instance**
- [ ] Choose **PostgreSQL 16**
- [ ] Fill in the settings:
  - Instance ID: `usertrack-db`
  - Password: Create a strong password and save it in a password manager
  - Region: **asia-southeast1**
  - Zonal availability: **Single zone** (cheaper for starters)
  - Machine type: **Shared core → 1 vCPU, 0.614 GB** (~$7/month, cheapest option)
  - Storage: **10 GB SSD**
...
- [ ] Note your **Connection name** shown on the instance overview page:
  ...
  ```
  You'll need this when deploying to Cloud Run.

---

## 6.4 Store Secrets in GCP Secret Manager

- [ ] Create your secrets via the terminal:
  ```bash
  echo -n "YOUR_DB_PASSWORD" | gcloud secrets create db-password --data-file=-
  echo -n "your-super-secret-jwt-key-at-least-32-chars!" | gcloud secrets create jwt-key --data-file=-
  ```
...
## 6.5 Create Artifact Registry Repository

- [ ] Create a Docker image repository:
  ```bash
  gcloud artifacts repositories create usertrack-repo \
    --repository-format=docker \
    --location=asia-southeast1 \
    --description="UserTrack Docker images"
  ```
- [ ] Your full image path will be:
  ```
  asia-southeast1-docker.pkg.dev/YOUR_PROJECT_ID/usertrack-repo/usertrack-api
  ```

---

## 6.6 Build and Push Your Docker Image
...
- [ ] Build the image:
  ```bash
  docker build -t asia-southeast1-docker.pkg.dev/YOUR_PROJECT_ID/usertrack-repo/usertrack-api:latest \
    ./backend/UserTrack.API
  ```
- [ ] Push it:
  ```bash
  docker push asia-southeast1-docker.pkg.dev/YOUR_PROJECT_ID/usertrack-repo/usertrack-api:latest
  ```
- [ ] Confirm: GCP Console → Artifact Registry → `usertrack-repo` → image appears with `latest` tag

---

## 6.7 Update Backend for Cloud SQL Connection

Cloud Run connects to Cloud SQL via a Unix socket — update `Program.cs` to handle this:

- [ ] Replace your database connection string setup with:
  ```csharp
  var dbPassword = Environment.GetEnvironmentVariable("DB_PASSWORD");
  var jwtKey = Environment.GetEnvironmentVariable("JWT_KEY");
  var instanceConnection = "YOUR_PROJECT_ID:asia-southeast1:usertrack-db";
  ...
  ```
- [ ] Replace `builder.Configuration["Jwt__Key"]` with `Environment.GetEnvironmentVariable("JWT_KEY")`
- [ ] Rebuild and push the updated image (repeat step 6.6)

---

## 6.8 Run Database Migrations via Cloud SQL Auth Proxy

Before deploying, apply EF Core migrations to Cloud SQL from your local machine:
...
- [ ] Download the Cloud SQL Auth Proxy:
  ...
  # Windows — download from the URL above replacing the filename with cloud-sql-proxy.x64.exe
  chmod +x cloud-sql-proxy
  ```
- [ ] Start the proxy in a **separate terminal tab**:
  ```bash
  ./cloud-sql-proxy YOUR_PROJECT_ID:asia-southeast1:usertrack-db --port 5433
  ```
...
## 6.9 Deploy to Cloud Run

- [ ] Deploy your backend:
  ```bash
  gcloud run deploy usertrack-api \
    --image asia-southeast1-docker.pkg.dev/YOUR_PROJECT_ID/usertrack-repo/usertrack-api:latest \
    --region asia-southeast1 \
    --platform managed \
    --allow-unauthenticated \
    --port 8080 \
    --add-cloudsql-instances YOUR_PROJECT_ID:asia-southeast1:usertrack-db \
    --set-env-vars ASPNETCORE_ENVIRONMENT=Production \
    --set-env-vars ASPNETCORE_URLS=http://+:8080 \
    --set-secrets DB_PASSWORD=db-password:latest \
    --set-secrets JWT_KEY=jwt-key:latest \
    --memory 512Mi \
    --cpu 1 \
    --timeout 3600
  ```
- [ ] After deployment you'll receive a URL like:
  ```
  https://usertrack-api-xxxx-as.a.run.app
  ```
  **Save this — it's your live production API!**

- [ ] Test it — visit in your browser:
  ```
  https://usertrack-api-xxxx-as.a.run.app/swagger
  ```
  You should see the Swagger UI with all endpoints listed.

- [ ] Check logs if anything fails:
  ```bash
  gcloud run logs read --service=usertrack-api --region=asia-southeast1 --limit=50
  ```

---

## 6.10 Configure SignalR for Cloud Run

Cloud Run supports WebSockets but needs tuning for long-lived SignalR connections:
...
- [ ] Update `Program.cs`:
  ...
  ```
- [ ] The `--timeout 3600` flag in the deploy command (step 6.9) already covers Cloud Run's side
- [ ] Rebuild, push, and redeploy after this change

---

## 6.11 Set Up Custom Domain (Optional)

- [ ] Go to **GCP Console → Cloud Run → your service → Custom Domains tab**
- [ ] Click **Add Mapping** → enter your domain (e.g., `api.usertrack.com`)
- [ ] Add the DNS records provided to your domain registrar
- [ ] GCP automatically provisions a free SSL/TLS certificate — no extra steps needed
...
- [ ] Add to **GitHub Secrets** (repo → Settings → Secrets → Actions → New secret):
  - `GCP_PROJECT_ID` — your Project ID
  - `GCP_SA_KEY` — paste the entire contents of `gcp-key.json`
  - **Delete `gcp-key.json` from your machine immediately after!**
...
- [ ] Create `.github/workflows/deploy.yml`:
  ...
  env:
    ...
    REGION: asia-southeast1
    SERVICE: usertrack-api
    IMAGE: asia-southeast1-docker.pkg.dev/${{ secrets.GCP_PROJECT_ID }}/usertrack-repo/usertrack-api
    ...
    deploy:
      ...
      steps:
        ...
        - name: Build and push Docker image
          run: |
            docker build -t $IMAGE:latest ./backend/UserTrack.API
...
- [ ] GCP Console → Cloud Run → new revision deployed automatically
...
- [ ] Update `network/RetrofitClient.kt`:
  ```kotlin
  private const val BASE_URL = "https://usertrack-api-xxxx-as.a.run.app/api/"
  // Or with custom domain:
  // private const val BASE_URL = "https://api.usertrack.com/api/"
  ```
- [ ] Update `network/LocationHubClient.kt`:
  ```kotlin
  .create("https://usertrack-api-xxxx-as.a.run.app/locationHub")
  ```
- [ ] Build and run — confirm login, groups, and real-time location all work in production
...
|---------|---------|-----------|
| Cloud Run | 512MB, ~1M requests/month | ~$0–5/month (generous free tier) |
| Cloud SQL | Shared core, 10GB SSD | ~$7–10/month |
| Artifact Registry | First 0.5GB free | ~$0.10/month |
| Secret Manager | First 6 secrets free | $0/month |
| Cloud DNS (optional) | 1 managed zone | ~$0.20/month |
| **Total** | | **~$8–15/month** |

> 💡 Cloud Run's free tier covers 2 million requests/month and 360,000 GB-seconds of memory — for a small app starting out, hosting is practically free!
...
- [ ] Cloud Run service deployed — Swagger UI loads at your `.run.app` URL
- [ ] SignalR timeout configured (3600s)
- [ ] GitHub Actions CI/CD pipeline deploys automatically on push to main
- [ ] Android app pointing to Cloud Run production URL
